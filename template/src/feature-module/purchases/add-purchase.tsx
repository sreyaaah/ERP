...
const AddPurchase = ({ onSuccess }: AddPurchaseProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedSupplier || orderTax === "" || items.length === 0) {
        setErrors({ general: "Please select a supplier, tax, and at least one product." });
        return;
    }
    setLoading(true);
    try {
        ...
        console.error("Submit purchase failed:", err);
      ...
      <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: "800px" }}>
        <div className="modal-content border-0">
          <div className="modal-header border-0 pb-0 pt-4 px-4">
            <h4 className="fw-bold modal-title" style={{ color: '#00263E' }}>Add Purchase</h4>
             <button
                    ref={closeBtnRef}
                    type="button"
